pipeline {
    ...
    // some block 
          ...
          //  sh 'codeql database analyze dvna javascript-code-scanning.qls --sarif-category=javascript --format=sarif-latest --output=dvna.sarif' //using the rules installed, analyse the goof app to see results and output in SARIF
            sh 'ls -la'
